...
#define com_port ((NS2416_UART_t)UART_0_BASE)	

int uart_init(int baudrate)
{
	unsigned int baud_divisor;
	
	baud_divisor = CFG_APB_UART_CLOCK / 16 / baudrate;	

	if(CFG_APB_UART_CLOCK > (baud_divisor * 16 * baudrate))
		baud_divisor +=((CFG_APB_UART_CLOCK/16 - baud_divisor * baudrate)*2 > baudrate) ? 1 : 0;
...
static void do_upload(void)
{
	...
	reply(E_OK);
	TRACE(KERN_UART,"Upload %d bytes at 0x%x\n", size, addr);
	tx(&cksum, 4);
	tx(addr, size);
}
...
static void do_download(void)
{
	...
	reply(E_OK);
	TRACE(KERN_UART,"Download %d bytes at 0x%x\n", size, addr);
}


int uart_boot(int boot_mode)
{
	while(1) {
		switch(wait_cmd()) {
		case XCMD_CONNECT:
