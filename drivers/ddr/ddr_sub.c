...
void ddr_reset()
{
	NS115_REG_WRITE(0x0582108c, REG_READ(0x0582108c) & 0xffffefff | 0x00000000);
	delay(10);
	NS115_REG_WRITE(0x0582108c, REG_READ(0x0582108c) & 0xffffefff | 0x00001000);
	delay(100);
}
...
void arbiter_init(unsigned int work_mode)
{
	...
	NS115_REG_WRITE(MEMMAX_BASE,(memmax_chip_bits<<28) | (memmax_cb_loc<<24) | (0xc<<16) | (0x2<<8) | (memmax_dram_type<<4) | 0x1);
	NS115_REG_WRITE(MEMMAX_BASE+0x00000020,0x00000002);
	NS115_REG_WRITE(MEMMAX_BASE+0x00000024,0x00400005);
	NS115_REG_WRITE(MEMMAX_BASE+0x00000030,0x00000002);
	NS115_REG_WRITE(MEMMAX_BASE+0x00000034,0x04000002);
	NS115_REG_WRITE(MEMMAX_BASE+0x00000060,0x04000001);
	NS115_REG_WRITE(MEMMAX_BASE+0x00000064,0x01000004);
	NS115_REG_WRITE(0x07200500,0x30);
}
